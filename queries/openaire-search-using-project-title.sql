...
FROM `coki-scratch-space.ioi.oi-name-deduced` AS B
  LEFT JOIN `academic-observatory.openaire.project20230817` AS A ON REGEXP_CONTAINS(A.title, B.infra_search_terms), UNNEST(funding) AS funder, UNNEST(granted) AS granted
