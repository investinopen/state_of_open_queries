WITH data_relation_organization AS (
  ...
  FROM `coki-ioi.openaire.openaire-relations-project20260513`
    LEFT JOIN `sos-datasources.openaire.organization_20260129`
    ON source=id
),
data_project_organization AS (
  ...
  FROM `sos-datasources.openaire.project_20260129` AS A
...
SELECT
  ...
  CAST(code AS STRING) AS GRANT_ID,
  ...
  CAST(NULL AS STRING) AS GRANT_CAT
FROM `coki-ioi.search_terms.search_terms_wellcome20260817` AS B
  LEFT JOIN data_project_organization AS A
  ON (REGEXP_CONTAINS(lower(A.summary), lower(B.terms_abstract)) 
      OR REGEXP_CONTAINS(lower(A.title), lower(B.terms_title))
      OR REGEXP_CONTAINS(lower(A.RECIPIENT), lower(B.terms_recipient))
      ),
  UNNEST(fundings) AS funder, UNNEST(granted) AS granted
