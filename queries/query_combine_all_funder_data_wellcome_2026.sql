...
data_coefficientgiving AS (
  SELECT
    ...
    CAST(NULL AS STRING) AS GRANT_CAT_2
  FROM `coki-ioi.search_terms.search_terms_wellcome20260817` AS B
    LEFT JOIN `coki-ioi.funder_data_2026.coefficientgiving` AS A 
    ON (REGEXP_CONTAINS(lower(A.grant_description), B.terms_abstract) 
        OR REGEXP_CONTAINS(lower(A.grant_title), B.terms_title)
        OR REGEXP_CONTAINS(lower(A.recipient_org_name), B.terms_recipient)
        )
...
data_nihr AS (
  SELECT
    ...
    CAST(NULL AS STRING) AS GRANT_CAT_2
  FROM `coki-ioi.search_terms.search_terms_wellcome20260817` AS B
    LEFT JOIN `coki-ioi.funder_data_2026.nihr` AS A 
    ON (REGEXP_CONTAINS(lower(A.grant_description), B.terms_abstract) 
        OR REGEXP_CONTAINS(lower(A.grant_title), B.terms_title)
        OR REGEXP_CONTAINS(lower(A.recipient_org_name), B.terms_recipient)
        )
...
UNION ALL
SELECT * FROM data_coefficientgiving
UNION ALL
SELECT * FROM data_nihr
